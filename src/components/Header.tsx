...
export function Header() {
  ...
  return (
    <header className="sticky top-0 z-40 bg-club-green text-club-cream shadow-md font-sans-ui">
      <div className="flex items-center justify-between gap-2 px-3 py-2.5 sm:gap-4 sm:px-6 sm:py-3 lg:grid lg:grid-cols-[1fr_auto_1fr] lg:px-8">
        <a href="#top" className="flex flex-1 items-center gap-2 sm:gap-3 lg:flex-none lg:justify-self-start">
          <Logo className="h-9 w-9 shrink-0 sm:h-12 sm:w-12" />
          <span className="text-xs font-semibold leading-tight sm:text-lg lg:text-xl">
            {club.name}
          </span>
        </a>
        <nav className="hidden shrink-0 justify-self-center gap-5 text-sm font-medium lg:flex">
          {navLinks.map((link) => (
